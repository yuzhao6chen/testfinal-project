...
try:
    ...
    def median_filter(data, window, need_two_end=False):
        w_len = window // 2 * 2 + 1
        ...
        for index in range(t_len):
            if not need_two_end and (index < half or index >= t_len - half):
                continue
            start = max(0, index - half)
            end = min(t_len, index + half + 1)
            ans[index] = float(np.median(values[start:end]))
        return ans
