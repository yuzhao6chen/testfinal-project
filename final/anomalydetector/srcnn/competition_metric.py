...
def reconstruct_label(timestamp, label):
    ...
    unique_timestamps = []
    unique_labels = []
    for ts, lb in zip(timestamp_sorted, label):
        if unique_timestamps and ts == unique_timestamps[-1]:
            unique_labels[-1] = max(unique_labels[-1], lb)
        else:
            unique_timestamps.append(ts)
            unique_labels.append(lb)

    timestamp_sorted = np.asarray(unique_timestamps, np.int64)
    label = np.asarray(unique_labels, np.int64)
    if len(timestamp_sorted) <= 1:
        return label

    diffs = np.diff(timestamp_sorted)
    positive_diffs = diffs[diffs > 0]
    if len(positive_diffs) == 0:
        return label
    interval = np.min(positive_diffs)
...
def evaluate_for_all_series(lst_timestamp_label_predict, delay=7, prt=True):
    ...
    rec = recall_score(labels, predicts, zero_division=0)
